...
| **Maintainer of the agent backend ships a `postinstall` that exfiltrates `OPENAI_API_KEY` from CI.** | Scaffolded `create-daloy` templates ship `ignore-scripts=true`, `minimum-release-age=1440`, and an empty `pnpm.onlyBuiltDependencies` allowlist. Template `_gitignore` excludes `.env*`. `verify:no-lifecycle-scripts` refuses any install-time hook on a published manifest. `step-security/harden-runner` on the publish workflow blocks egress from CI. |
